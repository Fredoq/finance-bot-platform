...
public sealed class TelegramBotPortTests
{
    ...
    public async Task Sends_message()
    {
        ...
        await item.Send(new TelegramText(100, "hello", [new TelegramRow([new TelegramButton("➕ Add account", "account.add", "primary")])], keys), default);
    ...
    public async Task Edits_message()
    {
        ...
        using var client = new HttpClient(lane) { BaseAddress = new Uri("https://api.telegram.org/") };
        var item = new TelegramBotPort(client, Options.Create(new TelegramBotOptions { Token = "token", BaseUrl = "https://api.telegram.org", TimeoutSeconds = 10 }));
        await item.Send(new TelegramEditText(100, 8, "hello", [new TelegramRow([new TelegramButton("↩ Back", "transaction.recent.back")])], keys), default);
    ...
    public async Task Retries_throttle()
    {
        ...
        using var client = new HttpClient(lane) { BaseAddress = new Uri("https://api.telegram.org/") };
        var item = new TelegramBotPort(client, Options.Create(new TelegramBotOptions { Token = "token", BaseUrl = "https://api.telegram.org", TimeoutSeconds = 10 }));
        DeliveryException error = await Assert.ThrowsAsync<DeliveryException>(() => item.Send(new TelegramText(100, "hello", [], keys), default).AsTask());
