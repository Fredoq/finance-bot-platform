...
public sealed class PostgresPersistenceTests : FinanceCoreRuntimeSuite
{
    ...
    private static MessageEnvelope<WorkspaceViewRequestedCommand> ViewEnvelope() => new(
        ...
        "finance-core",
        new WorkspaceViewRequestedCommand(new WorkspaceIdentity("actor-8", "room-8"), new WorkspaceProfile("Alex", "en"), new WorkspaceViewFrame("home", "{\"accounts\":[]}", ["account.add"]), new WorkspaceViewFreshness(false, false), DateTimeOffset.UtcNow));
}
