...
public sealed class MonthlySummaryRuntimeTests : FinanceCoreRuntimeSuite
{
    ...
    public async Task Returns_and_rejects_text()
    {
        ...
        await Publish(Envelope("actor-summary-text", "room-summary-text-side", string.Empty, "summary-text-side-home"));
        _ = await Take(queue, "summary-text-side-home");
        await Record(queue, "actor-summary-text", "room-summary-text-side", new EntryNote("income", "Cash", "7", "Salary", "summary-text-income", new DateTimeOffset(2026, 4, 20, 12, 0, 30, TimeSpan.Zero)));
        ...
        Assert.Equal(7m, Total(invalid.Payload.Frame.StateData, "USD", "income"));
        await Record(queue, "actor-summary-text", "room-summary-text-side", new EntryNote("expense", "Cash", "3", "Food", "summary-text-expense", new DateTimeOffset(2026, 4, 20, 12, 1, 30, TimeSpan.Zero)));
        await Publish(InputAt("actor-summary-text", "room-summary-text", "action", "account.add", "summary-text-action", new DateTimeOffset(2026, 4, 20, 12, 2, 0, TimeSpan.Zero)));
        MessageEnvelope<WorkspaceViewRequestedCommand> action = await Take(queue, "summary-text-action");
        Assert.Equal("summary.month", action.Payload.Frame.State);
        Assert.Equal("Use the buttons to change the month or go back", Error(action.Payload.Frame.StateData));
        Assert.Equal(7m, Total(action.Payload.Frame.StateData, "USD", "income"));
        Assert.Equal(3m, Total(action.Payload.Frame.StateData, "USD", "expense"));
        await Publish(InputAt("actor-summary-text", "room-summary-text", "action", "summary.month.back", "summary-text-back", new DateTimeOffset(2026, 4, 20, 12, 3, 0, TimeSpan.Zero)));
    ...
    private async Task Record(string queue, string actor, string room, EntryNote note)
    {
        ...
        if (view.Payload.Frame.State.EndsWith(".account", StringComparison.Ordinal))
        {
            ...
            {
                await Publish(Input(actor, room, "text", note.Category, $"{note.Id}-5"));
