...
public sealed class WorkspaceSqlTests : FinanceCoreRuntimeSuite
{
    ...
    public async Task Loads_accounts()
    {
        ...
        await Execute("insert into finance.user_account(id, actor_key, name, locale, created_utc, updated_utc) values (@id, @actor, @name, @locale, @created, @updated)", ("id", user), ("actor", "actor-sql"), ("name", "Alex"), ("locale", "en"), ("created", DateTimeOffset.UtcNow), ("updated", DateTimeOffset.UtcNow));
        await Execute("insert into finance.user_account(id, actor_key, name, locale, created_utc, updated_utc) values (@id, @actor, @name, @locale, @created, @updated)", ("id", user2), ("actor", "actor-sql-2"), ("name", "Sam"), ("locale", "en"), ("created", DateTimeOffset.UtcNow), ("updated", DateTimeOffset.UtcNow));
